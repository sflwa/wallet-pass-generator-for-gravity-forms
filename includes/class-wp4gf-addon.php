...
class WP4GF_Addon extends GFAddOn {
				...
				'fields' => array(
					...
					array(
						...
						'field_map' => array(
							...
							array( 'name' => 'barcode_value', 'label' => 'QR Code Data (Merge Tags supported)', 'required' => true ),
						),
					),
				),
			),
			array(
				'title'  => esc_html__( 'Form Specific Images (Server Paths)', 'wallet-pass-generator-for-gravity-forms' ),
				'fields' => array(
					array( 'name' => 'wp4gf_logo_path', 'label' => 'Logo Path (logo.png)', 'type' => 'text', 'class' => 'large', 'description' => 'Absolute path to a 320x100 PNG' ),
					array( 'name' => 'wp4gf_icon_path', 'label' => 'Icon Path (icon.png)', 'type' => 'text', 'class' => 'large', 'description' => 'Absolute path to a 58x58 PNG' ),
				),
			),
		);
	}
	...
	public function wp4gf_replace_download_link( $text, $form, $entry, $url_encode, $esc_html, $nl2br, $format ) {
		...
		$link = sprintf( '<a href="%s" class="wp4gf-download">%s</a>', esc_url( $url ), __( 'Download Pass', 'wallet-pass-generator-for-gravity-forms' ) );
